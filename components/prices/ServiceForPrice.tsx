...
const ServiceForPrice: FC<PriceProps> = ({
  ...
  // Список услуг, для которых не нужно показывать "від"
  const noVidServices = ['Стрижка кучерів', 'Тест пасма'];
  ...
  return (
    <ul className="flex flex-col w-full max-w-full lg:max-w-[850px]">
      <li className="hidden lg:flex text-[16px] font-bold text-[#eaeaea] ml-auto mr-[50px]">
        <p className="w-[120px] text-center">Експерт</p>
        <p className="w-[120px] text-center">Арт-директор</p>
        <p className="w-[120px] text-center">Топ Майстер</p>
      </li>

      {servicePrices.map((service, index) => {
        const showVid = !noVidServices.includes(service.serviceTitle);
            ...
            >
              ...
              >
                <div className="w-[120px] text-center">
                  {showVid ? 'від ' : ''}
                  {service.priceExpert} грн
                </div>
                <div className="w-[120px] text-center border-l-[0.5px] border-r-[0.5px] border-grey-light">
                  {showVid ? 'від ' : ''}
                  {service.priceArtDir} грн
                </div>
                <div className="w-[120px] text-center">
                  {showVid ? 'від ' : ''}
                  {service.priceTop} грн
                </div>
              </div>
